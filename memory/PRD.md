...
### Journal Notes 01–08 (Still & Moving Diaries) — 2026-06-25
- ✅ Extracted artist-supplied PDF "Kobi Israel Journal Notes 01-08" verbatim into `data/journalNotes.js` (`KOBI_AUTHORED_NOTES` + `KOBI_NOTE_BY_SLUG`). Each note carries: number, theme, title, subtitle, epigraph (in single quotes), date_location, full body as paragraph array, seo_title, meta_description, internal_links and tags. Total: ~470 body paragraphs across the 8 notes
- ✅ Slugs created exactly as specified: `/journal/cuba-as-mirror`, `/journal/the-man-waiting-for-the-bus`, `/journal/a-photograph-stops-time-but-not-memory`, `/journal/masculinity-as-theatre`, `/journal/the-football-field-and-the-white-line`, `/journal/the-chocolate-river`, `/journal/the-camera-given-by-moshe`, `/journal/the-viewer-as-detective`
- ✅ Author rule applied: every entry shows "By Kobi Israel" — Yaqov / Grossi / Yaqov Israel Grossi confirmed absent from all 8 entries and the /journal index
- ✅ Existing journal page visual language preserved: cards still use the same grid, typography hierarchy, theme labels, excerpts and "Read more" affordance via the existing `JournalCard` template; 8 authored notes replace the placeholder featured set + first 8 grid entries; remaining 7 placeholders kept for filter variety
- ✅ Each entry page renders: theme label, title, subtitle, epigraph (curly single quotes ‘ ’ + gold left rule), date / location, "By Kobi Israel" byline, full body paragraphs, related internal links section (3–6 per note), tag pills (6–8 per note)
- ✅ SEO per the PDF: `<title>` = artist's supplied SEO title ending in "Kobi Israel Journal"; `meta description` = artist's supplied copy (146–168 chars); canonical → `https://kobiisrael.life/journal/<slug>`; `meta robots=index, follow`; BreadcrumbList + Article JSON-LD attached
- ✅ Tested: testing_agent iteration_22 → 100% pass (all 8 entry pages verified: title, subtitle, epigraph, date/location, byline, ≥30 body paragraphs, ≥3 related links, ≥3 tags, exact verbatim opening phrases present, 0 occurrences of "Yaqov"/"Grossi"/placeholders, mobile 390x844 no overflow, full smoke regression across other 10 main pages still green including the Cuba YouTube embed)

### Journal Notes 01–08 — Cuba photographs wired as hero images — 2026-06-25
- ✅ Added `hero_image` + `hero_alt` to every entry in `KOBI_AUTHORED_NOTES` using the 5 Cuba photographs already on the site, distributed by tonal fit: Memory→CUBA.i1, Encounter→CUBA.i2, Stillness→CUBA.i3, Masculinity→CUBA.i4, Belonging→CUBA.i5, Water→CUBA.i1, Inheritance→CUBA.i2, Looking→CUBA.i3
- ✅ Updated `JournalEntry.jsx` hero section to render the photograph full-bleed (16:8 aspect, 90% opacity, bottom gradient into ki-bg) when `note.hero_image` is set; the "Hero image · to be confirmed by artist" fallback remains for any future un-illustrated notes
- ✅ Verified live: 8/8 journal entry pages serve their assigned Cuba photograph as the first `<img>` on the page (hash-checked: 7bp5yfi9_01, ok45ywq1_02, 8kvjmiyn_03, kvyze4pg_04, 3cvn8u5e_05 in the correct slug→image mapping)

### Journal — "Start Here: A Curated Reading Path" — 2026-06-25
- ✅ Extracted artist-supplied PDF ("Kobi Israel Journal Start Here Curated Reading Path") verbatim into `data/curatedReadingPath.js` — 12 entries in the artist's intended reading order (original_number, theme, title, excerpt, tags, slug, pending flag); cycles the 5 Cuba photographs as `hero_image`/`hero_alt` so every card has imagery
- ✅ New section on `/journal` between the hero and the "Writing as an Extension of the Image" intro: heading "A Curated Reading Path", descriptive paragraph in artist's voice, jump-to-full-index anchor (href=#journal-index), and a responsive 3-up grid of 12 article cards
- ✅ Each card: aspect-[4/5] image with bottom-gradient, "Note · NN" + theme label top, "Position NN of 12" bottom, title + excerpt + tag pills + "Read more" CTA — all routing to `/journal/<slug>`
- ✅ Full `data-testid` coverage: `journal-start-here`, `journal-start-here-heading`, `journal-start-here-jump-full`, `journal-start-here-list`, `journal-curated-card-<slug>`, `journal-curated-title-<slug>`, `journal-curated-readmore-<slug>`
- ✅ 5 of the 12 curated slugs map to existing entry pages (cuba-as-mirror, the-man-waiting-for-the-bus, the-football-field-and-the-white-line, the-chocolate-river, the-camera-given-by-moshe); the remaining 7 are marked `pending:true` in the data file and route to the existing NotFound page until the artist supplies their bodies — no fabricated content
- ✅ Tested: testing_agent iteration_23 → 100% pass (12 cards verified, jump anchor scrolls to #journal-index, cuba-as-mirror loads its full entry, pending slug correctly resolves to NotFound, all 10 main pages render with zero console errors, mobile 390x844 no overflow, backend 21/21 pytest still green)


### Image spread — distribute all 9 artist photographs across page heroes — 2026-06-25
- ✅ Added new `ARTIST_IMAGES` registry to `data/cubaImages.js` exposing the 4 non-Cuba artist photographs (Views02, Fragments/1700, Intimate Strangers/01, River 7.3) so they can be reused as hero/intro images on top-level pages, not only as their own project covers
- ✅ Reassigned 8 hero/intro slots in `data/site.js`: STILL_HERO → Views02; STILL_CURATORIAL_IMAGE → Fragments/1700; PRINTS_HERO → River 7.3; BOOKS_HERO → Fragments/1700; CV_HERO → Views02; JOURNAL_HERO → Intimate Strangers/01; JOURNAL_EDITORIAL_IMAGE → River 7.3; CONTACT_INTRO_IMAGE → Intimate Strangers/01
- ✅ Each of the 4 non-Cuba images now appears in 3 surfaces (its own project page + 2 page-level hero positions); Cuba.i1/i3/i4/i5 retained on homepage HERO, Moving, Archive, Contact heroes and on the Cuba project's dedicated surfaces
- ✅ Each new hero has thematic-fit alt text: Views→still photography + biographical military; Fragments→Books object + Tel-Aviv urban memory; River→prints landscape + journal reading path; Intimate Strangers→journal of encounters + intimate contact
- ✅ Cleaned up stale captions now that real artist imagery is in place: removed "Final hero image to be selected by artist" from `Journal.jsx`; removed "Final archive image to be selected by artist" from `Archive.jsx`; trimmed the CV hero caption to "Download CV placeholder · file to be supplied by artist"
- ✅ Tested: testing_agent iteration_24 → 100% pass (all 9 main-page first-section hero img URLs verified against expected artifact hashes; 3 secondary intro images verified on their pages; 5 project detail pages still serve their own cover; stale captions confirmed removed; 10-route navigation regression with zero console errors; mobile 390x844 no overflow; backend POST /api/inquiries still 201)


### Page-aware sharing images (og:image + twitter:image per route) — 2026-06-25
- ✅ Extended `lib/seo.js` `applyPageSeo` with `image` + `imageAlt` params; sets `og:image`, `og:image:alt`, `og:image:width=1600`, `og:image:height=1067`, `twitter:image`, `twitter:image:alt` on every public page. Falls back to a new `DEFAULT_OG_IMAGE` (Cuba.i1, the homepage signature) when a page does not supply one
- ✅ Wired each main page to pass its hero through SEO: Home→HERO, Still→STILL_HERO (Views02), Moving→MOVING_HERO (CUBA.i4), Prints→PRINTS_HERO (River 7.3), Books→BOOKS_HERO (Fragments/1700), CV→CV_HERO (Views02), Archive→ARCHIVE_HERO (CUBA.i5), Journal→JOURNAL_HERO (Intimate Strangers), Contact→CONTACT_HERO (CUBA.i3)
- ✅ Detail pages thread their own image: `/projects/:slug` uses `project.hero_image` (5 illustrated projects share their cover; un-illustrated projects fall back to the default); `/journal/:slug` uses `note.hero_image` (the 8 authored notes each have a Cuba photograph assigned)
- ✅ First-paint baked into `public/index.html`: homepage og:image, og:image:alt, og:image:width/height and twitter:image/twitter:image:alt all present before React hydrates, so LinkedIn / X / Slack / iMessage previews are correct on cold scrapes
- ✅ Tested: testing_agent iteration_25 → 100% pass (17 mapped routes verified; fallback verified on /projects/parisian-postcards; first-paint HTML verified; og:image:width=1600 + og:image:height=1067 verified; backend regression test added at `/app/backend/tests/test_og_image_regression.py`; POST /api/inquiries still 201; 10-route mobile 390x844 no overflow; zero console errors)
- ℹ️ Note for future edits to `public/index.html`: CRA dev server does not HMR the HTML shell; `supervisorctl restart frontend` is required for changes to that file to be served
